...
def schedule_checkin(flight_time, reservation):
    ...
    if checkin_time > current_time:
        ...
        m, s = divmod(delta, 60)
        h, m = divmod(m, 60)
        logger.info("Too early to check in.  Waiting {} hours, {} minutes, {} seconds".format(trunc(h), trunc(m), s))
        try:
            time.sleep(delta)
        except OverflowError:
            logger.error("System unable to sleep for that long, try checking in closer to your departure date")
            sys.exit(1)
...
def auto_checkin(reservation_number, first_name, last_name, verbose=False):
    ...
    for leg in body['bounds']:
        ...
        if date > now:
            ...
            logger.info("Flight information found, departing {} at {}".format(airport, date.strftime('%b %d %I:%M%p')))
